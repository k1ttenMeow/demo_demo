    ...
    >
      ...
      >
        ...
        <el-form-item label="周期" prop="cycle">
          <el-input v-model="form.cycle" placeholder="例如：30 天、90 天" style="width: 100%" />
        </el-form-item>
...
const handleDelete = async (row) => {
  try {
    ...
    const res = await request.delete(`/followup/plan/${row.id}`)
    if (res.code === 200) {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        if (form.id) {
          // 编辑
          res = await request.put('/followup/plan', form)
        } else {
          // 新增
          res = await request.post('/followup/plan', form)
        }
